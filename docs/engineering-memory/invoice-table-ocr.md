...
Pack hierarchy is pharmacy-owned medicine evidence. Catalog editing exposes base stock unit, pack conversions and selling-unit prices behind advanced progressive disclosure. Approved onboarding/import/photo/barcode sources may populate the same canonical maps, but sparse evidence must never erase them and generic package words must never create a conversion.
