...
When medicine-name anchors are incomplete or confused, finalized unique batch identifiers are stronger row-order anchors. Read each batch token's vertical image coordinate and order the finalized rows by those coordinates when every row's batch is visible; retain medicine geometry as fallback. Batch positions affect presentation order only and must never rewrite row values.
