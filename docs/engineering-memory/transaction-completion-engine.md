...
The repository contains the local single-context TCE foundation, Cash and Manual adapters, a production-shaped Simulator adapter, pending-payment queue state, daily numbering, idempotent provider events, duplicate-callback protection and linked undo/reversal records. Fast Record is wired into existing confirmed sales. Tenant merchant onboarding, tenant-bound request fields, capability-driven UI, multi-tenant simulator scenarios, secure credential storage, owner-facing Request & Verify queue screens, reconciliation hooks and official provider adapters remain future live-tested stages. No production credentials or provider integration exist.
