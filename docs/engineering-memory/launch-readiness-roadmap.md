...
| Multiuser safety | MS2-LT-067, 077 | IN PROGRESS | MS2-LT-067-A is owner-verified passed/frozen/protected: physical staging proved direct first-owner activation, durable credential survival across republish, PIN-only returning-owner authentication and pharmacy-bound Main App entry. Later staff invitations, shared truth, attribution, removal and device revocation remain under MS2-LT-077. |
| Permission and audit reliability | MS2-LT-066–069 | IN PROGRESS | Owner authentication and audit roots are protected. Export scope/redaction, identifier minimization, bounded history retention and non-endorsement safeguards are implemented with owner qualification pending; quiet-UI qualification remains. |
...
MS2-LT-067-A is the protected foundation: deterministic tenant resolution activates the first owner once, the owner creates a private PIN, and later access confirms the tenant-derived masked identity and accepts only that PIN. Activation values are digest-only, PINs are uniquely salted strong hashes in the protected workbook, sessions are opaque secure cookies, and messaging/platform bearer access is not a customer dependency. The controlled HTTPS physical-owner journey passed across a republish boundary on 2026-08-08; this does not authorize later staff or multiuser cases.

Deployment durability was proven as part of this gate. The first republish exposed an ephemeral local credential fallback when no dedicated admin workbook ID was present. The corrected startup uses the dedicated admin workbook or the already configured registry workbook and fails closed if durable persistence is unavailable. The recreated staging credential survived republish/restart and completed PIN-only repeat sign-in on the physical owner phone.
...
Locked unified-entry requirement: MS2-LT-073 must accept authenticated channel, QR and share-link entry through adapters into the same tenant/provisioning state machine. Unknown-pharmacy entry establishes verified owner identity/phone and exactly one tenant before first-owner activation; existing-pharmacy QR/link context routes only to the tenant-bound MS2-LT-077 invitation flow. Signed context is short-lived, non-enumerable and fail-closed on tamper, expiry, replay or mismatch; no customer sees internal IDs and no routine onboarding needs operator/developer work.

Locked owner-phone requirement: MS2-LT-077 must let the legitimate owner change the canonical registered phone through strong current-ownership verification and independent new-phone verification, preserving the pharmacy tenant while invalidating the old phone and re-evaluating sessions/devices/Quick PIN trust. MS2-LT-076 qualifies a verified-identity fallback when the old phone or Primary Owner PIN is unavailable so recovery is not circular. Changes are normalized, tenant-bound, audited without secrets and fail closed on any verification or cross-pharmacy mismatch.
