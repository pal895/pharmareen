...
Customer-entry signing is a distinct MS2.0 security boundary and cannot depend on whether an optional trusted edge routing key is deployed. `MS20_FRONT_DOOR_SIGNING_KEY` signs short-lived, one-use setup/invitation contexts; it is platform-managed infrastructure, never a customer/channel identity. The legacy routing key is accepted only for deployment compatibility. Missing signing configuration fails before issuing setup authority and cannot create partial tenant state.
