...
### Verification-delivery friction and repair (2026-08-09)

Physical evidence showed the code-entry screen without receipt of a code. Production inspection found one correctly normalized pending item, zero delivery attempts, zero sends and zero failures: the previous route equated a write to the deployment-local WhatsApp Web outbox with delivery, while no Baileys bridge consumed that outbox. New-owner verification now uses the official Meta WhatsApp authentication-template adapter and advances only when Meta returns a message ID. Missing credentials/template, transport errors and provider rejection return a truthful 503, remove the unusable challenge and keep the customer on the editable phone screen. The code screen includes a simple in-card back arrow to change the number. The legacy bridge/outbox remains available for its non-verification workflows but cannot assert OTP delivery. Physical retest is blocked until the official provider credentials and approved template are configured.

### Permanent MS2.0 account-independence correction (2026-08-09)

The preceding Meta verification proposal is superseded and must not be revived. Core pharmacy creation, owner identity, PIN protection, sign-in, sessions, device handling, staff membership and normal operation cannot depend on Meta, WhatsApp, SMS, email, Google, Apple or another external identity/messaging platform. `/start` is one MS2.0-owned form: pharmacy name, owner name, location, optional contact phone, and Primary Owner PIN. The server generates unrelated random durable pharmacy and owner IDs, issues one-use setup authority, persists the credential under the tenant, and returns a tenant-bound session. Phone is optional profile data only and changing it does not change credentials, sessions, membership or pharmacy identity.

The existing provider-dependent owner-recovery endpoint is disabled rather than silently reassigned. Provider-independent recovery/account protection is an explicit unresolved product-design requirement and must receive owner approval before implementation. Existing established pharmacies retain their current durable IDs and protected authentication; the new Owner ID registry column is additive and old phone-derived IDs remain stable legacy identities. Optional WhatsApp adapters remain non-core communication surfaces only.

### Creation-boundary migration and retry contract (2026-08-09)

Additive registry fields must resize an older Google Sheets grid before updating headers or appending records. Independent setup authority durably allocates the pharmacy and owner IDs before the registry write; retries reuse those identities, resume exact partial provisioning and cannot append a duplicate tenant. The browser retains the same setup authority after a recoverable failure. Every response is decoded defensively so a proxy/runtime plain-text error becomes a short customer-safe retry message, never a JSON-parser exception. The first Mwangaza attempt failed during the pre-append header migration and therefore created no registry row; its creation boundary remains unpassed until the repaired deployment succeeds physically.

Customer-entry signing is a distinct MS2.0 security boundary and cannot depend on whether an optional trusted edge routing key is deployed. `MS20_FRONT_DOOR_SIGNING_KEY` signs short-lived, one-use setup/invitation contexts; when a dedicated key is absent, a versioned domain-separated SHA-256 derivation of the already protected MS2.0 `SESSION_SECRET` provides the same boundary without exposing or duplicating secret material. Both are platform-managed infrastructure, never a customer/channel identity. The legacy routing key is accepted only for deployment compatibility. Missing signing configuration fails before issuing setup authority and cannot create partial tenant state.

All Google Sheets-backed additive schemas, including owner credentials and sessions, expand the physical worksheet grid before updating headers or values. Provisioning is a resumable saga: if registry, tenant identity and credential commits exist but the final session/context completion failed, an exact matching live-onboarding record may be adopted only after verification of its stored Primary Owner PIN. The canonical existing pharmacy/owner IDs win, the registry is not appended again, and the new setup authority is rebound and completed. Mere matching names or locations never authorize adoption.

The public setup-form start is not a durable state transition. It issues a short-lived signed random context locally and performs no Google Sheets write. The first Create request claims that context durably with stable nonce-derived pharmacy/owner IDs and replay protection before any registry mutation. This removes a remote persistence dependency from form entry without weakening expiry, non-enumerability, idempotency, PIN-gated partial-tenant adoption or tenant isolation.

## Primary Owner recovery — locked architecture (2026-08-10)

Primary Owner recovery is provider-independent and has two proofs only. A trusted owner device presents its enrolled high-entropy HttpOnly device credential; a four-digit Quick PIN is never ownership proof. A lost device uses the server-generated human-readable Recovery Key. Recovery Keys contain cryptographic randomness, are stored only as salted PBKDF2 verifiers, are shown only at enrollment/rotation, are attempt-limited with a 15-minute lock after five failures, and cannot be derived from or replaced by pharmacy name, owner, location, phone, messaging or email.

Successful recovery replaces the salted Primary Owner PIN verifier, revokes all prior owner sessions, rotates the Recovery Key, marks devices and Quick-PIN trust for recheck, records non-secret security audit state, preserves the pharmacy ID and all business data, and issues one fresh owner session. New pharmacy UX is `Create pharmacy → Save Recovery Key → Main App`; lost-device UX is `Recover my pharmacy → Recovery Key → new Primary Owner PIN`; trusted-device recovery is two steps.

The pre-recovery Mwangaza tenant uses one controlled migration script bound in source to its exact authoritative pharmacy and owner IDs. It can only add a missing Recovery Key verifier, cannot change the PIN, cannot target another tenant, and permanently refuses once recovery is enrolled. It is not a public or general-purpose admin reset endpoint.
