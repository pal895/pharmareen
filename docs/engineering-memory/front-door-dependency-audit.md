...
The integrated contract is `app/front_door.py`. It defines explicit entry states, fixed roles, owner-authority boundaries and signed adapter contexts. Main App consumes the decision through authenticated operations bootstrap. Setup cards are removed whenever durable resolution is unavailable or the tenant still needs explicit restore/new classification.
