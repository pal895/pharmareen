...
The first package crop remained too broad and low-resolution for a carton displayed inside a laptop page: another pair produced no match then a correct match. The package-focused path now crops the tighter lower-center area, upscales it to a stable OCR width, reads normal and binary layouts, and separately reads its upper section for medicine identity and strength. This is a generic package-composition repair, not a Metronidazole rule; whole-frame union, unique-identifier safety, cancellation, bounded client upload, local execution and zero-token behavior remain protected.

The repaired Camera consistency checkpoint passed after `74f7f69`. Repeated acquisition produced the same canonical Metronidazole 400 mg draft and catalog-authoritative Current stock 34. All three sections retained the result, correction/reason stayed blank, and no mutation occurred. Camera is protected; File and microphone remain distinct source responsibilities and must use different saved medicines in separate sessions.

The distinct File checkpoint uses a normal downloaded Ibuprofen 200 mg PNG with package-only evidence: 24 tablets, batch `IBU-200C` and expiry `2028-12`. It enters the same bounded local OCR and normalized evidence boundary as Camera/Photo, but File acquisition and unsupported-file isolation remain source-specific responsibilities. Saved stock 28 must come only from the matched catalog record; corrected stock 27 remains owner-entered and optional Reason stays blank. File must not enter onboarding or require special transport. Microphone remains separate.

File live acceptance passed on Android with the normal Ibuprofen PNG. It populated canonical Ibuprofen 200 mg and catalog-authoritative stock 28, preserved owner control of correction/optional reason, applied 28 to 27 once, and independently persisted stock 27 with retained commercial values and no draft change. No onboarding route, special transport, second approval, manual Sync or pending duplicate appeared. File is protected; guided microphone remains the final distinct Stock Fix source checkpoint.

The final distinct source checkpoint is guided microphone with saved Co-Amoxiclav at trusted stock 24. Optional Reason exposed a pre-live safety gap: saying Confirm immediately after corrected stock could bypass the promised spoken review. Guided voice now treats the first Confirm on an unreviewed draft as a request for the deterministic Medicine → Stock → Reason review, marks all three sections reviewed, and resumes listening only when local speech ends. A second explicit Confirm is required to execute. The isolated live test will request stock 23 with Reason blank, require the review to say `not provided`, and verify one immediate durable update with no pending duplicate or canonical-name replacement.

The first Android guided-microphone attempt exposed a capture-layer failure before medicine matching: the control stayed in `Speak` and showed no transcript or failure after repeated speech. Speech capture now exposes interim/final words in the composer, explicitly stops when speech ends or after eight seconds, and reports when no completed transcript arrives. An unmatched or ambiguous medicine preserves `I heard …` on the card and waits for an intentional Mic retry; it cannot silently loop. This makes pronunciation evidence observable without weakening catalog-first matching or mutating stock.
