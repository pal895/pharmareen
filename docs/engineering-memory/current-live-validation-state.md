...
Approve the currently open controlled two-row shelf review exactly once. Verify the success message, then open the Pharmacy Catalog and search Prednisolone and Septrin separately. Confirm the catalog count moves from 27 to 29 and both saved rows retain their reviewed stock, commercial, supplier, batch, expiry, and shelf values. Do not repeat the shelf scan during this checkpoint.
