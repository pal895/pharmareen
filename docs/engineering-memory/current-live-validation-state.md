...
Deploy the direct-camera shelf acquisition update once. Display `fixtures/shelf-photo-c3-camera.png` full-screen on another screen, then on the phone choose `+` -> `Shelf photo` -> `Take photo`. Verify rear-camera acquisition, Capture, Retake/Use photo preview, and the resulting unsaved two-row Metronidazole/Ibuprofen review. Do not approve during this checkpoint.
