(function (root, factory) {
  ...
  function titleCaseDrugName(value) {
    return normalizeSpaces(value)
      ...
      .replace(/\s+\d+(?:\.\d+)?\s*$/i, "")
      .replace(/\b(eye drops?|ear drops?|tablets?|tabs?|capsules?|caps?|strips?|boxes?|box|bottles?|vials?|amps?|ampoules?|ampules?|syr|syrups?|creams?|ointments?|gels?|drops?|sachets?|packs?|tubes?|inhalers?|injs?|injections?|susp|suspensions?|pieces?|units?)\b/gi, "")
      .trim()
      .split(" ")
      .filter(Boolean)
  ...
  function splitFastSaleLine(line) {
    ...
    const pattern = /([A-Za-z][A-Za-z' -]*?)\s+(?:x\s*)?(\d+(?:\.\d+)?)(?:\s+(eye drops?|ear drops?|tablets?|tabs?|capsules?|caps?|strips?|boxes?|box|bottles?|vials?|amps?|ampoules?|ampules?|syr|syrups?|creams?|ointments?|gels?|drops?|sachets?|packs?|tubes?|inhalers?|injs?|injections?|susp|suspensions?|pieces?|units?))?(?:\s+(cash|mpesa|m-pesa|card|credit))?(?=\s+[A-Za-z][A-Za-z' -]*?\s+(?:x\s*)?\d|$)/gi;
  ...
  function canonicalUnit(value) {
    ...
    if (/^amps?$|^ampoules?$|^ampules?$/.test(unit)) return "ampoule";
    if (/^creams?$/.test(unit)) return "cream";
    if (/^tubes?$/.test(unit)) return "tube";
    if (/^syr$|^syrups?$/.test(unit)) return "syrup";
    ...
    if (/^injs?$|^injections?$/.test(unit)) return "injection";
    if (/^susp$|^suspensions?$/.test(unit)) return "suspension";
  ...
  function extractUnit(text) {
    const match = String(text || "").match(/\b(eye drops?|ear drops?|tablets?|tabs?|capsules?|caps?|strips?|boxes?|box|bottles?|packs?|vials?|amps?|ampoules?|ampules?|creams?|ointments?|gels?|drops?|tubes?|syr|syrups?|sachets?|inhalers?|injs?|injections?|susp|suspensions?|pieces?|units?)\b/i);
