...
> Historical foundation handoff. Derive every checkpoint only from `../MS2.0_MASTER_LIVE_TEST_SEQUENCE.md`. Embedded “next” statements below are evidence, not a competing plan.
...
## Shared medicine matcher continuation - 2026-07-13

- Live evidence: persisted alias search handled `Zinc syrup` and compact `zincsyrup`, but `zinc sirup` returned zero because catalog filtering used a separate stricter implementation.
- Broad repair: `medicineMatcher.js` is the single local confidence-ranked recognition engine reused by catalog search, Pharmacy/Source Brain, Operations Chat, onboarding/import, sales, restock, stock enquiry, and speech-recognized text. It supports normalization, aliases/brands/generics, form variants, partial and reordered input, spelling/phonetic/OCR tolerance, strength-aware ranking, and safe ambiguity without mutation or AI calls.
- Focused matcher/catalog verification and connected architecture verification pass. Run one representative live `zinc sirup` search, then close this matching behavior class and resume the next unfinished onboarding option rather than testing more equivalent spellings.
- Do not declare onboarding complete: preserve CSV, Excel, remaining bulk-paste coverage, manual creation, learning during sales, scheduled invoice/photo consistency, barcode/scan onboarding, and onboarding Medicine Action Card checks. Mic Test 2 stays in the official voice section after these catalog/onboarding checks and before general sales progression; it must pass recognized speech through the same local matcher and include first-use/mobile/offline/editable-card/zero-token checks.
