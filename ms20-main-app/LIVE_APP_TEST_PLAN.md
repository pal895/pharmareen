...
### Revised MS2-LT-067-A — First-owner activation, repeat sign-in and pharmacy-bound session

Internal prerequisite (not an owner action and never repeated inside the live test): an authorized deployment operator privately configures protected platform administration and prepares one pharmacy-bound activation invitation. Production remains fail-closed when that internal protection is absent. Never disclose its credential or the invitation contents, and never use platform administration as owner authentication.
...
The prior admin-secret and WhatsApp-OTP proposals were withdrawn before owner completion. This is the only active owner case. Before inviting the owner, the authorized operator privately deploys the tested build, confirms unauthenticated `/api/ms20/auth/session` returns 401, and creates one owner activation QR for the exact test pharmacy through protected administration. Those are infrastructure evidence, not owner instructions. The owner scans the invitation with the physical phone over controlled HTTPS, confirms the named pharmacy and owner, creates the private PIN, and arrives in the Main App. After sign-out, the owner opens MS2.0, enters the registered phone and PIN, and arrives in the Main App again. Capture only safe screens: the pre-PIN identity page without invitation contents, post-activation Main App, signed-out sign-in page, and successful repeat Main App. Never capture activation contents, PIN, cookie, session value, platform credential, or private logs. Then stop; automated evidence alone cannot pass the checkpoint.
