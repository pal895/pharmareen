...
The prior admin-secret, operator-invitation and WhatsApp-OTP proposals were withdrawn before owner completion. This is the only active owner case. On the physical phone over HTTPS, open the deployed MS2.0 URL. If the bound pharmacy has no owner credential, the application automatically shows first-owner activation. Confirm only that the displayed pharmacy and owner are correct, create and confirm a private PIN, and tap **Activate and open Main App**. Sign out, reopen MS2.0, enter the registered phone and PIN, and confirm Main App entry. Capture only safe screens: first-owner identity before entering a PIN, post-activation Main App, signed-out sign-in page, and successful repeat Main App. Never capture a PIN, cookie, session value, platform credential or private logs. A wrong identity, unavailable setup, or ordinary sign-in appearing before initialization is a stop/fail. Automated evidence alone cannot pass the checkpoint.
