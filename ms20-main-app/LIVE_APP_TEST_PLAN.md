...
2. **Original 055-B — Refund, money only:** UNDERLYING LOGIC OWNER-VERIFIED on 2026-07-31; clarity retest pending. Sale 4 recorded Ibuprofen x1 Cash with stock 19; Refund 2 reversed KES 18, restored zero stock, retained immutable reciprocal linkage, and the catalog independently remained 19. Owner friction showed no obvious selected state, technical `inventory` wording and ambiguous bare `Refund 2` naming. The shared presentation now gives the selected choice a filled green/checkmarked state, asks `Should this medicine go back into stock?`, shows `Money back` and `Stock added back`, and leads with `Refund for Sale 4` while retaining `Refund record #2` only as secondary audit identity. Repeat only this clarity case with a new eligible sale before advancing to 055-C.
