...
**MS2-LT-055-K / Direct Command 5 — Credit routing only:** this is the only active owner case. Use the Mic to create and confirm one fresh `Ibuprofen one tablet cash` sale and note its Sale number. Then use the Mic and say `Credit sale number [number]`. Verify the existing shared Credit review opens for that immutable completed sale with quantity 1, previously adjusted 0, remaining 1 → 0, account credit KES 18, stock added back 0, payment impact `Create KES 18 account credit; no cash refund`, original sale completed and the explicit review-only/no-change message. Do not confirm Credit. Capture the completed sale receipt and full Credit review, then stop. Typing is fallback only if voice is unavailable; no Credit record, stock, cash refund, financial posting or queue mutation is permitted in this case.
