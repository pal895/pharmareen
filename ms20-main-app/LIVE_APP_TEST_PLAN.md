...
- Mic Test 2 is now the active stage before general sales progression. It must cover local speech text flowing through the shared matcher, aliases/misspellings/partials, first-use/mobile/offline behavior where supported, editable Medicine Action Card output, and zero-token matching for known medicines. Start with only the safest voice boundary: tap `Mic`, allow permission if requested, say `Show me`, confirm the complete 35-item catalog opens through the shared controller, close it, and confirm the chat and catalog remain unchanged. Do not approve, save, or perform a sale in this first voice checkpoint.
