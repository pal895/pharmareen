...
**MS2-LT-055-L / Direct Command 6 — Safe linked Undo:** this is the only active owner case. Use the Mic to create and confirm one fresh `Ibuprofen one tablet cash` sale and note its Sale number and stock after sale. Then use the Mic and say `Undo sale number [number]`. Verify an Undo review opens for the correct immutable completed sale and explicitly says nothing has changed, the original will remain in history and one linked reversal will be created only after confirmation. Capture that review, then tap Confirm Undo exactly once. Verify one `Undo Sale [number]` result, KES 18 reversed, stock restored exactly once to its pre-sale value, original Sale retained with one linked reversal, adjustment controls blocked, and one queued/local action with no duplicate effect. Capture the result, original linked-sale card and catalog stock, then stop. Do not repeat the command. Typing is fallback only if voice is unavailable.
