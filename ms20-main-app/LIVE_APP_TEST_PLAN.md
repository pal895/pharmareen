...
5. **Original 055-E — partial/full and repeat safety:** **PARTIAL OWNER PASS — FOCUSED PROTECTION RETEST REQUIRED.** Owner evidence proves Sale 4 x3 Cash, stock 16 → 13, Return record #7 for 1/KES 18/stock 1, Return record #8 for 2/KES 36/stock 2, immutable reciprocal links and final catalog stock 16. The fully adjusted sale correctly created no further record or effect, but its entry controls were visually silent. The shared engine now exposes one availability result to every Return/Refund/Credit entry, renders `This sale has already been fully adjusted. No stock or money changed.`, disables all three adjustment controls, and preserves numeric zero instead of `Not recorded`. Retest only this final protection state; do not repeat the passed sale or split returns.
