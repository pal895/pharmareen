...
**MS2-LT-055-I / Direct Command 3 — Confirmed Direct Return:** this is the only active owner case. If the Sale 2 Return review from 055-H remains open, continue there; otherwise use voice `return sale 2` only to re-enter the same review. Tap Confirm Return exactly once. Verify one permanent Return record linked reciprocally to immutable completed Sale 2, quantity 1, KES 18 reversed against Cash, stock restored exactly 15 → 16, remaining quantity 0, fully-adjusted controls disabled and no duplicate record, stock, money or queue effect. Capture the Return receipt/detail, Sale 2 detail with reciprocal link and protection, and catalog stock 16, then stop. Confirmation must use the existing shared adjustment engine; no command-specific confirmation path or later direct-command case is authorized.
