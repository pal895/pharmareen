...
The prior admin-secret and WhatsApp-OTP proposals were withdrawn before owner completion. This is the only active owner case. Pull/restart through `bash start.sh`, run `tests/test_owner_auth.py tests/test_admin_access_control.py`, and confirm unauthenticated `/api/ms20/auth/session` returns 401. In the protected pharmacy-onboarding screen create one owner activation QR for the exact test pharmacy. Do not screenshot or paste its contents. Scan it with the physical phone over the controlled HTTPS staging entrance; require the activation page to name the correct pharmacy and owner before PIN entry. Create the private PIN, require the Main App, sign out, then use `/main-app/sign-in` with the registered phone and PIN and require the Main App again. In the same phone browser open `/api/ms20/auth/session` and require authenticated owner, correct pharmacy and fixed capabilities without activation token, PIN, cookie, session token, platform secret or credential. Capture focused tests, 401, onboarding QR screen without encoded value, pre-PIN identity page, post-activation Main App, successful repeat sign-in and safe session facts; then stop.
