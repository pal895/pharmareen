...
**MS2-LT-055-M / Direct Command 7 — Last-sale resolution:** this is the only active owner case. Do not create another sale. First capture the current catalog stock 13 and queue count. Tap Mic and say `Open last sale`. Verify the latest persisted immutable completed Sale 2 opens, showing Ibuprofen x1 tablet, KES 18 Cash, historical stock after sale 12, completed status, its one linked Undo reversal and blocked Refund/Return/Credit controls. Confirm that catalog stock remains 13 and queue count is unchanged. Do not tap any adjustment or issue any other last-sale action. Typing is permitted only if voice again produces `cell`; record the fallback and stop for screenshots of the command transcript, Sale 2 detail, catalog stock and queue count.
