...
MS2-LT-055-K is the latest owner-verified passed/frozen/protected checkpoint. Exact numbered Credit commands route before medicine parsing through the shared priority router and existing Credit review root with no mutation. MS2-LT-055-L is the only active owner case. Numbered Undo commands open a review-only completed-sale card; explicit Confirm Undo delegates once to the existing idempotent TCE reversal, retains the original sale, restores base stock only on new reversal creation, queues one linked local-first action and exposes the reciprocal reversal on the original sale. Adjusted and already-undone sales are blocked. Permanent interaction priority remains **Voice first → fast tap/action second → typing last**.
