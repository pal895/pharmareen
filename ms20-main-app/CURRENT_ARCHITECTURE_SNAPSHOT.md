...
Revised MS2-LT-067-A uses `app/owner_auth.py` as the shared customer authentication root and no longer depends on messaging or an operator invitation. The deployment's explicit pharmacy ID must resolve to one exact active trusted registry record. `GET /main-app/sign-in` checks the durable credential state: only an uninitialized pharmacy receives first-owner setup; initialized pharmacies receive normal sign-in. `POST /api/ms20/auth/owner/bootstrap` accepts only a PIN, derives pharmacy/owner/phone on the server, reloads durable state under the auth lock, rejects an existing pharmacy credential, and rolls back on persistence failure. A PIN requires at least eight characters with letters and numbers and is stored only as PBKDF2-SHA256 with a unique salt and 600,000 iterations. Setup and later phone-plus-PIN sign-in issue the same opaque 15-minute Secure/HttpOnly/SameSite=Strict cookie. Five-attempt/15-minute lockout and central pharmacy/role/session enforcement remain unchanged. Platform-admin routes remain separately protected and fail closed.
