...
- Word Owner Document is the next active checkpoint. It is deliberately an editable working copy for notes, corrections and owner review rather than an analysis workbook, fixed phone copy or technical transfer file. DOCX generation validates the same immutable snapshot, preserves 35 medicines across seven editable landscape pages, uses truthful missing-value handling and remains pharmacy-isolated, deterministic and zero-AI. `python-docx` structural validation passes; local render QA is unavailable because LibreOffice is not installed.
