...
The existing owner-auth service remains the protected credential/session root. `../app/services/front_door_persistence.py` stores the remaining bounded entry truth in the same protected platform workbook without raw phones, device keys, invitation tokens, PINs or cookies. Operations bootstrap projects the front-door decision, and Main App clears stale onboarding cards before showing unavailable or unresolved state. The durable root now includes memberships, fixed roles, one-use invitation/QR/link replay protection, attribution, devices/revocation, owner-phone/account recovery rechecks, trusted offline resume, failed-provisioning resume, legal/privacy acceptance versions, collision-safe Community identity, pharmacy-pooled Loyalty/referral identity and device-independent seat/billing authority. Deeper operational behavior remains in its original future checkpoints; this implementation-order gate does not pass them.
...
`src/services/exportCompliance.js` is the shared pre-builder compliance root. The default private pharmacy scope preserves complete canonical operational data. The optional safer-sharing scope removes cost price, supplier, barcode, batch, shelf and precise location while retaining every medicine. Pharmacy mismatch and unregistered formats fail closed. Browser history is pharmacy-keyed, sanitized to minimal workflow metadata and retained for 30 days; generated binaries remain device-only. The same policy owns the asset/licence register and makes third-party application names descriptive compatibility guidance, never an endorsement claim.
...
MS2-LT-055-O is the latest owner-verified passed/frozen/protected checkpoint, and checkpoint 55 is PASS/PROTECTED. Owner evidence proves safe interception of incomplete/unsafe commands, read-only nonexistent references, and stable linked-Undo blocking across direct and tapped Return/Refund/Credit entry. `saleReversalFor()` matches runtime and permanent identities inside `startSaleAdjustment()`, so no adjustment entry can bypass a linked reversal. Stock remained 13 and Queue remained 7 with no mutation. The owner has authorized forward movement to checkpoint 56. Permanent interaction priority remains **Voice first → fast tap/action second → typing last**.
...
Revised MS2-LT-067-A uses `app/owner_auth.py` as the shared customer authentication root and no longer depends on messaging or an operator invitation. The deployment's explicit pharmacy ID must resolve to one exact active trusted registry record. `GET /main-app/sign-in` uses one canonical state decision: no credential for that pharmacy is `uninitialized`; exactly one credential whose normalized phone matches the current registry record is `initialized`; unavailable, duplicate, or mismatched state fails closed with 503. Errors are never converted into a sign-in screen. `POST /api/ms20/auth/owner/bootstrap` accepts only a PIN, derives pharmacy/owner/phone on the server, reloads durable state under the auth lock, rejects an existing pharmacy credential, and rolls back on persistence failure. A PIN requires at least eight characters with letters and numbers and is stored only as PBKDF2-SHA256 with a unique salt and 600,000 iterations. Setup and later phone-plus-PIN sign-in issue the same opaque 15-minute Secure/HttpOnly/SameSite=Strict cookie. Five-attempt/15-minute lockout and central pharmacy/role/session enforcement remain unchanged. Platform-admin routes remain separately protected and fail closed.

Production tenant selection is request-scoped, not a per-pharmacy process environment setting. A trusted edge performs the indexed hostname-to-pharmacy lookup established by provisioning and signs the pharmacy ID plus a short-lived timestamp with one platform routing key; the application verifies that signature before loading the exact active registry record. Missing, expired, forged, unknown or ambiguous bindings fail closed. `PHARMAREEN_DEFAULT_PHARMACY_ID` is retained only for isolated development compatibility. A Replit/dev registry with exactly one active pharmacy may infer that sole record; zero or multiple records return 503.

`/main-app/` is the deterministic authentication entry boundary, not an unconditional static-file route. It resolves the same tenant and canonical owner state as bootstrap: uninitialized routes to first-owner activation, initialized without a valid session routes to owner sign-in, and a valid owner session bound to that pharmacy serves the Main App. Invalid, cross-pharmacy, duplicate, mismatched or unavailable state fails closed. Essential home-row guidance uses responsive wrapping rather than ellipsis.

Owner authentication and operations onboarding are distinct state machines. Successful owner activation establishes the durable credential and secure pharmacy-bound session; the Main App's browser-local onboarding flag only tracks business details/catalog readiness and must say `Operations setup needed`, never imply owner activation failed. Primary Owner PIN remains the master credential: 8+ characters with letters and numbers, uniquely salted PBKDF2-SHA256 at 600,000 iterations, required for new-device trust and stronger verification. A future optional Quick PIN is exactly four digits, device-bound, hashed/protected, attempt-limited, revocable, unable to establish ownership, and reset only after Primary Owner PIN verification. Primary recovery requires verified registered-owner identity, revokes/re-evaluates sessions and trusted-device Quick PINs, and fails closed; Quick PIN/device trust belongs after the device/session controls, while production identity recovery belongs to autonomous provisioning and channel/scale/recovery qualification.

Operations readiness is now rehydrated after authentication from `/api/ms20/operations/bootstrap`, which requires the owner session, matches it to the trusted routed registry pharmacy and reads that pharmacy's durable Sheets catalog. A non-empty durable catalog resumes initialized operations and removes stale onboarding cards; a durably empty pharmacy may begin setup. Unavailable or tenant-mismatched durable state fails closed and never resets or silently reopens setup. Browser setup/catalog caches are tenant-scoped, and the shared offline-sync and pronunciation-memory roots are rebound to the authenticated pharmacy. Live-validation order remains independent: runtime state selects the customer journey, while owner evidence selects only the next unproven boundary.

The durable catalog read accepts both the legacy `Master_Stock` representation and the authenticated pharmacy's operational inventory representation. It deliberately excludes the broad cross-inventory safety scan used by stock protection, so one tenant's inventory can neither initialize nor populate another tenant's Main App.

Pre-durable Main App builds stored completed setup/catalog only in unscoped browser storage and a process-memory placeholder. The upgrade bridge accepts that legacy workspace exactly once only when an authenticated browser has both its old completion flag and a non-empty catalog. The server binds it to the routed registry pharmacy, writes `MS20_Operations_State`, and subsequent readiness comes only from that pharmacy-scoped Sheets record. New browsers without legacy completion evidence continue through normal onboarding.

The customer boundary is permanent: platform-admin configuration is never an owner live-test action. Missing admin configuration keeps `/admin` fail-closed and never produces a weak/default credential or customer bypass. The ordinary first-owner path is open MS2.0 for the exact server-bound uninitialized pharmacy → confirm identity and create PIN → automatic Main App entry; repeat sign-in is open MS2.0 → submit registered phone plus PIN → automatic Main App entry. Automatic state detection, hashing, persistence, security checks, cookie creation and redirects do not add client actions.
...
Established legacy tenants retain their existing durable owner IDs, but phone is no longer an identity source for new accounts. New owners and pharmacies receive independent random MS2.0 IDs; optional contact phone is profile data only. Repeat sign-in is routed by the trusted pharmacy context and submits only the Primary Owner PIN. The server verifies canonical initialized state and rejects cross-pharmacy results.
...
The customer front door now has complete internal entry workflows over that root. Direct link/app entry produces a one-use MS2.0-owned new-pharmacy handoff into owner PIN creation; staff use owner-issued expiring invitation links, protected staff credentials, device-bound durable sessions and the same Main App/catalog without onboarding. Bootstrap returns actor identity/role for operational attribution. Owner access management projects members, devices, Community identity, pooled Impala Coins and truthful seat/subscription qualification. Contact-phone updates do not affect identity or sessions. Quick PIN trust, session/device revocation, bounded Loyalty earning/redemption and Community posting/moderation stay tenant-bound and fail closed. Provider-independent account recovery remains an explicit owner-design requirement; external legal and scale qualification remain separate unchanged Master gates.
