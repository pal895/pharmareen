...
MS2-LT-055-J is the latest owner-verified passed/frozen/protected checkpoint. The priority grammar accepts exact numbered Refund commands before medicine parsing and reuses the persisted completed-sale resolver and shared Refund review root; incomplete and unsafe lookalikes remain rejected. MS2-LT-055-K is the only active owner case: exact `credit sale N` and `credit sale number N`, with numeric or bounded spoken numbers, reuse the same priority router, persisted completed-sale resolver and shared Credit review root, with confirmation and all mutation outside this routing-only stage. Permanent interaction priority remains **Voice first → fast tap/action second → typing last**; typing is fallback, never the primary workflow.
