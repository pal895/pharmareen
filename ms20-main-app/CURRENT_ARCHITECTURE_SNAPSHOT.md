...
- Picture-assisted Stock Fix has its own isolated controlled-fixture registry and reuses the shared acquisition fingerprint boundary without changing ordinary medicine-photo onboarding. Recognized package evidence supplies identity only; trusted current stock always comes from the matched Pharmacy Catalog record, and corrected stock/reason remain owner-entered before the shared execution policy runs.
