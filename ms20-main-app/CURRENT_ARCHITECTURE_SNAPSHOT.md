...
Long shared editable cards preserve voice target visibility through the common render boundary. Before capture, `voiceViewportAnchor.js` records the exact selected field, caret, nearest scroll container and viewport coordinate. Microphone startup, listening, interim/final transcript and validation rerenders restore that anchor and bypass normal chat-bottom scrolling. Approval, discard or intentional navigation clears the retained edit anchor. Main Operations Chat Activity Compaction is owner-validated and protected; the viewport/focus checkpoint remains open pending focused owner evidence. The post-validation product-secrecy and quiet-UI audit remains pending.
