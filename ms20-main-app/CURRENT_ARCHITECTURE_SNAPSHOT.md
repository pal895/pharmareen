...
Revised MS2-LT-067-A uses `app/owner_auth.py` as the shared customer authentication root and no longer depends on messaging delivery. Admin-protected pharmacy onboarding creates a cryptographically random, 15-minute, single-use first-owner activation invitation bound to the exact pharmacy, owner and fixed owner role. Only the token digest is persisted. The QR/link places the raw value in a URL fragment; the activation page immediately removes the fragment, sends the value once in a same-origin POST body, applies `Referrer-Policy: no-referrer`, uses no browser storage and clears PIN fields. A PIN requires at least eight characters with letters and numbers, is stored only as PBKDF2-SHA256 with a unique salt and 600,000 iterations, and is protected by five-attempt/15-minute lockout. Activation and later phone-plus-PIN sign-in issue the same opaque 15-minute Secure/HttpOnly/SameSite=Strict cookie. Central authorization continues rejecting missing, wrong-pharmacy, wrong-role, expired and revoked sessions. Platform bearer access remains emergency/bootstrap only; messaging adapters remain optional future delivery channels, not owner-authentication dependencies.

The customer boundary is permanent: platform-admin configuration is a one-time, private deployment-operator prerequisite and never an owner live-test action. Missing configuration keeps `/admin` fail-closed and must never produce a weak/default credential or customer bypass. The ordinary owner activation path is exactly invitation open/scan → confirm identity and create PIN → automatic Main App entry; repeat sign-in is open MS2.0 → submit registered phone plus PIN → automatic Main App entry. Both remain at three client actions or fewer. Automatic inspection, hashing, persistence, security checks, cookie creation and redirects do not add client actions. Every future client workflow must follow the same maximum-three-step rule unless a documented security or legal requirement makes it impossible.
