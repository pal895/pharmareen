...
MS2-LT-055-K is the latest owner-verified passed/frozen/protected checkpoint. Exact numbered Credit commands route before medicine parsing through the shared priority router, persisted completed-sale resolver and shared Credit review root. The protected routing-only boundary creates no Credit record, stock restoration, cash refund, financial posting or queue mutation; original sale history remains immutable and completed. No later owner case is active until explicit authorization. Permanent interaction priority remains **Voice first → fast tap/action second → typing last**; typing is fallback, never the primary workflow.
