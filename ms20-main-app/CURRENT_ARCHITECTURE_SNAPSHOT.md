...
- Stock fix remains a protected safe-queue workflow rather than a direct catalog mutation. Its shared validation requires canonical saved identity, trusted current-stock agreement, a different non-negative whole corrected value and an audit reason; queued actions carry the signed adjustment and `queued_not_applied` truth.
...
- Stock Fix keeps the fast owner path to three main choices: Confirm, a single stateful Read control, and More. Medicine selection fills trusted saved stock when there is one safe catalog match. Owner copy uses simple English; queue truth remains review-first and does not imply saved stock changed.
