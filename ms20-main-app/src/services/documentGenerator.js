...
export function buildPrintHtml(model) {
  return `<!doctype html><html><head><meta charset="utf-8"><title>${xml(model.title)}</title><style>@page{size:A4 landscape;margin:10mm}body{font:12px Arial;color:#19332f}h1{color:#086c5c;margin:0 0 6px}.meta{margin:0 0 16px;color:#536b66}table{width:100%;border-collapse:collapse;font-size:9px}th{background:#086c5c;color:white;text-align:left}th,td{padding:5px;border:1px solid #c9d8d4;vertical-align:top}tr:nth-child(even){background:#f1f7f5}footer{margin-top:10px;color:#536b66}@media screen{body{max-width:1200px;margin:24px auto;padding:16px}}@media print{button{display:none}thead{display:table-header-group}}</style></head><body><button onclick="window.print()">Print inventory</button><h1>${xml(model.title)}</h1><p class="meta">${xml(model.pharmacyName)} | ${xml(model.branch)} | ${xml(model.location)}<br>Generated ${xml(model.generatedKenya)} Africa/Nairobi</p><table><thead><tr>${COLUMNS.map(([, label]) => `<th>${xml(label)}</th>`).join("")}</tr></thead><tbody>${model.rows.map((row) => `<tr>${COLUMNS.map(([key]) => `<td>${xml(row[key])}</td>`).join("")}</tr>`).join("")}</tbody></table><footer>${model.rows.length} canonical medicine records. Generated locally by MS2.0 with no AI formatting.</footer></body></html>`;
}
