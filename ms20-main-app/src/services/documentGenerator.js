...
export function buildCanonicalInventoryExport({ pharmacy, items, generatedAt = new Date() }) {
  ...
  return Object.freeze({
    ...
    title: "Pharmacy Inventory", columns: COLUMNS, rows: Object.freeze(rows)
  });
}
...
export function buildPrintHtml(model) {
  ...
  const mobileCards = model.rows.map((row, index) => `<details class="medicine-card" data-search="${xml(Object.values(row).join(" ").toLowerCase())}"${index === 0 ? " open" : ""}><summary><span><strong>${xml(row.medicine || "Unnamed medicine")}</strong><small>${xml([row.strength, row.form, row.unit].filter(Boolean).join(" · ") || "Details not recorded")}</small></span><span class="quick-facts"><b>Stock ${xml(row.stock === "" ? "—" : row.stock)}</b><small>KES ${xml(row.sellingPrice === "" ? "—" : row.sellingPrice)}</small></span></summary><div class="details-grid">${detailRows(row)}</div></details>`).join("");
