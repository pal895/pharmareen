...
export { EXPORT_FORMATS } from "./exportFormatMetadata.js";
...
function csvFieldValue(key, value) {
  ...
  if (key === "barcode" && /^0\d+$/.test(text)) return `'${text}`;
  return value;
}
function csvCell(value) {
  const text = typeof value === "number" ? String(value) : protectCsvSpreadsheetText(value);
