...
export function medicineFinderClientScript(index) {
  ...
  return `(()=>{const index=${json},input=document.querySelector("#medicine-search"),count=document.querySelector("#review-count"),filter=document.querySelector("#medicine-filter"),cards=new Map([...document.querySelectorAll(".medicine-card")].map(card=>[card.dataset.finderId,card]));const norm=value=>String(value||"").normalize("NFKD").replace(/[\\u0300-\\u036f]/g,"").toLowerCase().replace(/[^a-z0-9%]+/g," ").trim();const distance=(a,b)=>{const row=Array.from({length:b.length+1},(_,i)=>i);for(let i=1;i<=a.length;i++){let previous=row[0];row[0]=i;for(let j=1;j<=b.length;j++){const saved=row[j];row[j]=Math.min(row[j]+1,row[j-1]+1,previous+(a[i-1]===b[j-1]?0:1));previous=saved}}return row[b.length]};const score=(entry,wanted)=>{if(!wanted)return 1;if(entry.canonical===wanted)return 100;if(entry.canonical.startsWith(wanted))return 90;if(entry.aliases.includes(wanted))return 85;if(entry.search.includes(wanted))return 75;const terms=entry.search.split(" "),queries=wanted.split(" ");if(queries.some(query=>/\\d/.test(query)&&!terms.includes(query)))return 0;return queries.every(query=>terms.some(term=>query.length>=4&&distance(query,term)<=Math.max(1,Math.ceil(query.length*.2))))?60:0};const apply=()=>{const wanted=norm(input.value),selected=filter.value;const scored=index.filter(entry=>selected==="all"||selected==="az"||entry.flags[selected]).map(entry=>({entry,value:score(entry,wanted)})).filter(result=>result.value>=54).sort((a,b)=>b.value-a.value||a.entry.name.localeCompare(b.entry.name));const visible=new Set(scored.map(result=>result.entry.id));cards.forEach((card,id)=>card.hidden=!visible.has(id));count.textContent=scored.length+" of "+index.length+" medicines shown · Tap a medicine to view every field."};input.addEventListener("input",apply);filter.addEventListener("change",apply);const request=action=>window.opener?.postMessage({type:"ms20:finder-request",action},location.origin);document.querySelector("#finder-scan").addEventListener("click",()=>request("barcode"));document.querySelector("#finder-voice").addEventListener("click",()=>request("voice"));window.addEventListener("message",event=>{if(event.source!==window.opener||event.origin!==location.origin||event.data?.type!=="ms20:finder-result")return;if(event.data.query){input.value=event.data.query;filter.value="all";apply();input.scrollIntoView({behavior:"smooth",block:"center"})}});apply()})()`;
}
