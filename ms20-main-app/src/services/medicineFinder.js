...
export function medicineFinderClientScript(index, { bridgeId = "", initialQuery = "", initialMessage = "" } = {}) {
  ...
  const query = JSON.stringify(String(initialQuery || ""));
  const message = JSON.stringify(String(initialMessage || ""));
  return `(()=>{const index=${json},bridgeId=${bridge},input=document.querySelector("#medicine-search"),count=document.querySelector("#review-count"),status=document.querySelector("#finder-status"),filter=document.querySelector("#medicine-filter"),cards=new Map([...document.querySelectorAll(".medicine-card")].map(card=>[card.dataset.finderId,card]));input.value=${query};status.textContent=${message};const norm=value=>String(value||"").normalize("NFKD").replace(/[\\u0300-\\u036f]/g,"").toLowerCase().replace(/[^a-z0-9%]+/g," ").trim();const distance=(a,b)=>{const row=Array.from({length:b.length+1},(_,i)=>i);for(let i=1;i<=a.length;i++){let previous=row[0];row[0]=i;for(let j=1;j<=b.length;j++){const saved=row[j];row[j]=Math.min(row[j]+1,row[j-1]+1,previous+(a[i-1]===b[j-1]?0:1));previous=saved}}return row[b.length]};const score=(entry,wanted)=>{if(!wanted)return 1;if(entry.canonical===wanted)return 100;if(entry.canonical.startsWith(wanted))return 90;if(entry.aliases.includes(wanted))return 85;if(entry.search.includes(wanted))return 75;const terms=entry.search.split(" "),queries=wanted.split(" ");if(queries.some(query=>/\\d/.test(query)&&!terms.includes(query)))return 0;return queries.every(query=>terms.some(term=>query.length>=4&&distance(query,term)<=Math.max(1,Math.ceil(query.length*.2))))?60:0};const apply=()=>{const wanted=norm(input.value),selected=filter.value;const scored=index.filter(entry=>selected==="all"||selected==="az"||entry.flags[selected]).map(entry=>({entry,value:score(entry,wanted)})).filter(result=>wanted?result.value>=54:result.value>0).sort((a,b)=>selected==="az"?a.entry.name.localeCompare(b.entry.name):b.value-a.value||a.entry.name.localeCompare(b.entry.name));const visible=new Set(scored.map(result=>result.entry.id));cards.forEach((card,id)=>card.hidden=!visible.has(id));count.textContent=scored.length+" of "+index.length+" medicines shown · Tap a medicine to view every field."};const receive=data=>{if(data?.type!=="ms20:finder-result"||data.bridgeId!==bridgeId)return;if(data.message!==undefined)status.textContent=data.message;if(data.query){input.value=data.query;filter.value="all";apply();input.scrollIntoView({behavior:"smooth",block:"center"})}};let channel=null;if(bridgeId&&"BroadcastChannel" in window){channel=new BroadcastChannel("ms20-finder-"+bridgeId);channel.addEventListener("message",event=>receive(event.data))}const request=action=>{status.textContent=action==="barcode"?"Requesting camera…":"Requesting microphone…";const requestData={type:"ms20:finder-request",action,bridgeId};if(window.parent!==window&&typeof window.parent.__ms20FinderRequest==="function")window.parent.__ms20FinderRequest(requestData);else if(channel)channel.postMessage(requestData);else if(window.opener)window.opener.postMessage(requestData,"*");else status.textContent="Return to MS2.0 and reopen Print. The finder connection is unavailable."};input.addEventListener("input",()=>{status.textContent="";apply()});input.addEventListener("search",()=>{if(!input.value){filter.value="all";status.textContent="";apply()}});filter.addEventListener("change",apply);document.querySelector("#finder-scan").addEventListener("click",()=>request("barcode"));document.querySelector("#finder-voice").addEventListener("click",()=>request("voice"));document.querySelector(".action.secondary").addEventListener("click",event=>{if(window.parent!==window&&typeof window.parent.__ms20FinderRequest==="function"){event.preventDefault();window.parent.__ms20FinderRequest({type:"ms20:finder-request",action:"close",bridgeId})}});window.addEventListener("message",event=>{if(event.source===window.opener)receive(event.data)});apply()})()`;
}
