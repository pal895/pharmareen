...
| 59 | Google Sheets pharmacy persistence | Provision/read/write isolated pharmacy sheets and recover safely when unavailable. | 2, 20 | Implemented — awaiting owner live test | **MS2-LT-059-A connected Sheets/registry readiness is OWNER-VERIFIED PASSED/FROZEN/PROTECTED.** The first 059-B run safely showed pending 7, Not synced, no conflict, backend detected, Sheets connected and Baileys confirmed, with Queue unchanged at 7. It also exposed Confirm/Correct controls and hid the idempotent guidance, violating the intended read-only boundary. The root repair gives SyncReviewCard visible read-only/idempotent guidance plus Read and Close only. **Corrected MS2-LT-059-B is the only active case.** | No | 15 min | 2026-08-03 owner screenshots; `/live/readiness`; pharmacy registry schema check; Main App Check system/Sync review; read-only action regression; onboarding/sheets tests; TRAIN |
...
- **Category:** Integration Validation
- **Current status:** Implemented — awaiting owner live test
- **Repository evidence:** 2026-08-03 owner screenshots; `/live/readiness`; pharmacy registry schema check; Main App Check system/Sync review; read-only action regression; onboarding/sheets tests; TRAIN
