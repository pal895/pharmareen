...
| 67 | Authentication, roles and access controls | Enforce owner/admin/branch authorization and minimum-necessary access across UI, routes and downloads. | 2, 58–65 | Implemented — awaiting owner live test | **Revised MS2-LT-067-A first-owner activation/repeat sign-in is the only active owner case.** Protected onboarding creates one cryptographically random, 15-minute, single-use QR/link bound to the exact pharmacy and owner. The fragment-held value is stored only as a digest; the owner creates a strongly hashed private PIN and later signs in with registered phone plus PIN. Both paths issue the same opaque 15-minute HttpOnly session bound to pharmacy and owner role; missing, wrong-pharmacy, wrong-role, expired and revoked access fail closed. Messaging and platform bearer access are not customer authentication dependencies. | No | 15 min | `app/owner_auth.py`; `/admin/onboard`; `/main-app/activate`; `/main-app/sign-in`; `/api/ms20/auth/owner/*`; secure session cookie; focused owner-auth tests; ARCH; compliance plan |
...
- **Category:** Security / Privacy / Compliance Validation
- **Current status:** Implemented — awaiting owner live test
- **Repository evidence:** `app/owner_auth.py`; `/admin/onboard`; `/main-app/activate`; `/main-app/sign-in`; `/api/ms20/auth/owner/*`; secure session cookie; focused owner-auth tests; ARCH; compliance plan
- **Implementation commit(s):** Repository evidence not yet available.
- **Primary implementation files/modules:** `app/actor_context.py`; `app/routes/admin.py`
- **Owner live-test evidence:** **Revised MS2-LT-067-A first-owner activation/repeat sign-in is the only active owner case.** Protected onboarding creates one cryptographically random, 15-minute, single-use QR/link bound to the exact pharmacy and owner. The fragment-held value is stored only as a digest; the owner creates a strongly hashed private PIN and later signs in with registered phone plus PIN. Both paths issue the same opaque 15-minute HttpOnly session bound to pharmacy and owner role; missing, wrong-pharmacy, wrong-role, expired and revoked access fail closed. Messaging and platform bearer access are not customer authentication dependencies.; source: `app/owner_auth.py`; `/admin/onboard`; `/main-app/activate`; `/main-app/sign-in`; `/api/ms20/auth/owner/*`; secure session cookie; focused owner-auth tests; ARCH; compliance plan
- **PASS / PROTECTED confirmation:** Not applicable.
