...
Allowed implementation states are exactly: `PASS / PROTECTED`, `Implemented — awaiting owner live test`, `Partial implementation`, `Planned / approved`, `External qualification`, and `Deprecated with repository evidence`. A PASS requires owner evidence. Automated tests alone never create protected status.
...
| 10 | Catalog browse/search/edit | Browse 35 canonical medicines, search safely, edit one draft, approve/discard and persist. | 2 | PASS / PROTECTED | Passed | Yes | 10 min | CVS; Catalog Search Mic evidence; catalog verifier |
...
| 28 | Stock-out and demand-risk prediction | Anticipate likely stock-outs using canonical movement, lead time and confirmed unit truth. | 21, 27, 29 | Planned / approved | Not started | No | 15 min | OI; LATP approved improvements |
...
| 48 | Future operational documents | Generate supplier orders, GRNs, reconciliation, expiry and supplier reports from stored truth. | 29, 39–47 | Planned / approved | Not started | No | 20 min | OM Documents; LATP Improvement 1 |
...
| 51 | TCE Fast Record | Record owner-confirmed transactions immediately with daily numbering and exact one-time stock/finance effects. | 14, 50 | PASS / PROTECTED | Passed | Yes | 10 min | CVS; TCE |
| 52 | Request & Verify success | Queue non-cash request, continue serving and complete only after verified success. | 50–51 | PASS / PROTECTED | Passed | Yes | 12 min | CVS; TCE; TCE/UI verifiers |
...
| # | Checkpoint | Objective | Prerequisite(s) | Implementation state | Owner validation | Protected | Est. live test | Evidence/document source |
|---:|---|---|---|---|---|---|---|---|
| 65 | Pharmacy/branch isolation | Prevent cross-pharmacy catalog, learning, notification, payment, export and credential leakage. | All data workflows | PASS / PROTECTED | Passed in protected workflows and automated gates | Yes | 15 min | CVS; ARCH; isolation tests/verifiers |
| 66 | Idempotency, audit and duplicate prevention | Ensure retries/callbacks/imports/actions cannot duplicate mutations and every correction remains traceable. | 14–20, 51–56 | PASS / PROTECTED | Passed across protected workflows | Yes | 15 min | CVS; TCE; activity/export/offline verifiers |
...
The only current open checkpoint is **#13 Editable-card voice viewport/focus**. Do not implement or test #14 onward because #14–#21 are already protected. After #13 passes with owner evidence, continue at the smallest not-yet-passed checkpoint in canonical order: **#22 Direct analytics commands** is later, but **#54 Payment failure/cancellation notification** is explicitly the next isolated checkpoint inherited from the protected notification/TCE sequence. Complete #54, then resume numeric order among unpassed checkpoints, honoring prerequisites.
