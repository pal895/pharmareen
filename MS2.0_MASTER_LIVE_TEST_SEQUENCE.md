...
| 67 | Authentication, roles and access controls | Enforce owner/admin/branch authorization and minimum-necessary access across UI, routes and downloads. | 2, 58–65 | Implemented — awaiting owner live test | **Revised MS2-LT-067-A owner sign-in/session is the only active owner case.** A registered owner signs in with their WhatsApp number and a one-time code delivered through the internal Baileys bridge. The shared root binds the resulting short-lived HttpOnly session to the registry pharmacy and owner role; unauthenticated, wrong-pharmacy, wrong-role, expired and revoked access fail closed. No platform or session secret is shown or stored by client code. Platform bearer access remains emergency/bootstrap only. | No | 15 min | `app/owner_auth.py`; `/main-app/sign-in`; `/api/ms20/auth/owner/*`; secure session cookie; registry/Baileys roots; focused owner-auth tests; ARCH; compliance plan |
...
- **Category:** Security / Privacy / Compliance Validation
- **Current status:** Implemented — awaiting owner live test
- **Repository evidence:** `app/owner_auth.py`; `/main-app/sign-in`; `/api/ms20/auth/owner/*`; secure session cookie; registry/Baileys roots; focused owner-auth tests; ARCH; compliance plan
- **Implementation commit(s):** Repository evidence not yet available.
- **Primary implementation files/modules:** `app/actor_context.py`; `app/routes/admin.py`
- **Owner live-test evidence:** **Revised MS2-LT-067-A owner sign-in/session is the only active owner case.** A registered owner signs in with their WhatsApp number and a one-time code delivered through the internal Baileys bridge. The shared root binds the resulting short-lived HttpOnly session to the registry pharmacy and owner role; unauthenticated, wrong-pharmacy, wrong-role, expired and revoked access fail closed. No platform or session secret is shown or stored by client code. Platform bearer access remains emergency/bootstrap only.; source: `app/owner_auth.py`; `/main-app/sign-in`; `/api/ms20/auth/owner/*`; secure session cookie; registry/Baileys roots; focused owner-auth tests; ARCH; compliance plan
- **PASS / PROTECTED confirmation:** Not applicable.
