...
| 67 | Authentication, roles and access controls | Enforce owner/admin/branch authorization and minimum-necessary access across UI, routes and downloads. | 2, 58–65 | Implemented — awaiting owner live test | **Revised MS2-LT-067-A automatic first-owner initialization/repeat sign-in is the only active owner case.** For one exact active server-bound registry pharmacy, setup appears only while no durable owner credential exists. The client cannot select pharmacy, owner or registered phone. Successful atomic PIN setup permanently closes self-activation; later access is registered phone plus PIN. Both paths issue the same opaque 15-minute HttpOnly session bound to pharmacy and owner role; missing configuration/store, repeated setup, wrong-pharmacy, wrong-role, expired and revoked access fail closed. Platform administration remains separate. | No | 15 min | `app/owner_auth.py`; `/main-app/sign-in`; `/api/ms20/auth/owner/bootstrap`; `/api/ms20/auth/owner/pin`; secure session cookie; focused owner-auth tests; ARCH; compliance plan |
