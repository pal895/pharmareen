...
Notifications Quiet State, Pending Review Unread State and Review Import Action Routing are owner-validated, passed and regression-protected. Paste List raw input becomes a pending review only when the owner taps `Review list`; that deterministic parsing boundary creates the editable draft and its notification together. The compact `Review import` action reopens that same draft, and cancel/approve removes its linked alert. The next isolated Notifications checkpoint is Low-Stock Alert.
