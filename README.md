...
Infrastructure-only setup must never appear as a pharmacy-owner task. The platform-admin credential protects internal administration only, is never used for owner activation or sign-in, and remains fail-closed when absent. For a deployment bound to one exact active registry pharmacy, the shared owner-authentication root automatically offers first-owner setup only while that pharmacy has no durable owner credential. Exactly one pharmacy-and-phone-matching credential produces normal sign-in; unavailable, duplicate, or registry-mismatched state fails closed and is never hidden behind a sign-in page. The server derives pharmacy and owner identity from the deployment configuration plus trusted registry; the client cannot select either. The first successful atomic setup permanently closes self-activation and all later access uses registered phone plus private PIN.
